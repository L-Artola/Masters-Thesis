...
# Distribution of $ denominated variables. Boxcox transformation for skewness. Only do if we have a problem with with scaling individual variables in modeling 
```{r}

skewness(combined_quaterly_data_2.1$scaled_asset5)
#skewness(combined_quaterly_data_2.1$ernast5)
#skewness(combined_quaterly_data_2.1$lnlsgr5)

test <- scale(combined_quaterly_data_2.1$asset5)
skewness(test$asset5)
head(test$asset5)
```
...
# Correlation matrix
#data.cor <- combined_quaterly_data_FL %>%
#        select(!c(cert,DATE))
#cor(data.cor)
...
#colSums(is.na(combined_quaterly_data_L31.2))
...
# Stepwise logit
```{r}
# Full model without stepwise
head(short_train_10)

log <- glm(solvency ~., data = short_train_10, family = binomial) %>%
...
probabilites <- log %>%
        predict(step_test, type = "response")
predicted.classes <- ifelse(probabilites > 0.5, "pos", "neg")
mean(predicted.classes == step_test$solvency)
